...
#include<vector>
/*
NAPOMENA: većina koda je samo kopija koda iz zadatka 2.
Ovde su samo dodate određene funkcionalnosti (u kodu je označeno šta je izmenjeno/dodato)
*/
...
class Igrac : public Posmatrac {
	...
	Igra* igra; // igrači će u terminologiji projektnog uzorka posrednik biti "kolege", a 
	// igra je posrednik. Kada se igrač pomeri, on ne treba da o tome dikretno obavesti sve druge igrače,
	// niti čak mora da zna ko su drugi igrači ili postoji li uopšte još neki igrač. Njegovo je samo da 
	// svoju promenu saopšti posredniku, a posrednik će o tome obavestiti sve ostale!
public:
	...
	bool pomeriSe(SmerKretanja smer); // ista stvar, ovo ćemo kasnije implementirati...
	friend class Igra;
};
...
class BrojacIgraca : public Posmatrac {
	int brIgraca = 0;
public:
	void azuriraj(Igra* igra); // isto kao u slučaju igrača... implementacija je data nakon
...
public:
	...
	bool pomeriIgracaNa(Igrac* igrac, int novoI, int novoJ) {
		// metoda će vraćati true/false u zavisnosti od toga da li je pomeranje bilo uspešno.
		if (!validnoPolje(novoI, novoJ, N, M) || mapa[novoI][novoJ] != TipPolja::Cisto) {
			return false;
		}
		int staroI = igrac->i;
		int staroJ = igrac->j;
		
		mapa[staroI][staroJ] = TipPolja::Cisto;
		mapa[novoI][novoJ] = TipPolja::Igrac;
		igrac->i = novoI;
		igrac->j = novoJ;
		obavestiPosmatrace(); // Ovim igrač "posredno" obaveštava ostale igrače (pa i samog sebe, što
		// ne samo da nije neipsarvno, već je u trenutnoj implementaciji i potrebno) o svom porezu.

		// Ovaj primer bismo lako mogli da proširimo time da igrači imaju korisničko ime i da omogućimo
		// da igrači međusobno (ciljno jednom drugom igraču) šalju poruke ili neke resurse. 
		return true;
	}
...
bool Igrac::pomeriSe(SmerKretanja smer) {
	int di = 0;
	int dj = 0;
	switch (smer) {
	...
	int potencijalnoI = i + di;
	int potencijalnoJ = j + dj;
	
	return igra->pomeriIgracaNa(this, potencijalnoI, potencijalnoJ); // pomeriIgracaNa će ažurirati vrednosti
	// i, j, ukoliko je potez validan.
}
