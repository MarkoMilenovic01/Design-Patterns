...
class RTFCitac {
public:
	static const char _EOF = '&'; /* npr. znak za kraj fajla */
...
void RTFCitac::parsirajRTF(Dokument *dokument) {
    ...
    while ((znak = dokument->dajNaredniToken()) !=  _EOF) {
        switch (znak) {
            case PARA:
...
class Klijent {
public:
	ASCIITekst napraviASCIITekst(Dokument *dokument){
...
int main() {
	...
	auto asciiText = klijent->napraviASCIITekst(dokument);
